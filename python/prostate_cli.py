...
"""
DR-Prostate-CLI
전립선 질환 EAU 가이드라인 AI Agent - Ollama + RAG 기반 독립 CLI
"""
...
from prostate_agent import ProstateAgent
...
{Fore.MAGENTA}╔════════════════════════════════════════════════════════════════╗
║                        DR-Prostate-CLI                        ║
║              Prostate Diseases EAU Guidelines AI Agent        ║
║                                                                ║
║                 Powered by Ollama + RAG + EAU 2025           ║
╚════════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
"""
        print(banner)
...
{Fore.YELLOW}Usage:{Style.RESET_ALL}
  ...
  {Fore.GREEN}dr-prostate query "question"{Style.RESET_ALL}
    Ask prostate diseases related questions
    Example: dr-prostate query "What are PSA levels for prostate cancer screening?"

  {Fore.GREEN}dr-prostate chat{Style.RESET_ALL}
    Start interactive mode
    
  {Fore.GREEN}dr-prostate init{Style.RESET_ALL}
    Initialize system and install model
    
  {Fore.GREEN}dr-prostate status{Style.RESET_ALL}
    Check system status
    
  {Fore.GREEN}dr-prostate config{Style.RESET_ALL}
    Edit configuration file
    
  {Fore.GREEN}dr-prostate --help{Style.RESET_ALL}
    Show this help

{Fore.YELLOW}Example Questions:{Style.RESET_ALL}
  • "What are the PSA levels for prostate cancer screening?"
  • "What are the risk factors for prostate cancer?"
  • "How to manage BPH symptoms?"
  • "What are the side effects of hormone therapy?"
  • "When should prostate biopsy be performed?"

{Fore.YELLOW}Configuration:{Style.RESET_ALL}
  • Config file: ~/.dr-prostate/config.json
  • Log files: ~/.dr-prostate/logs/
  • Cache directory: ~/.dr-prostate/cache/
"""
        print(help_text)
    ...
    def init_agent(self) -> bool:
        """에이전트 초기화"""
        try:
            print(f"{Fore.BLUE}Initializing prostate agent...{Style.RESET_ALL}")
            ...
            # 에이전트 생성
            self.agent = ProstateAgent(self.config)
            
            # 초기화
            if not self.agent.initialize():
                print(f"{Fore.RED}Prostate agent initialization failed{Style.RESET_ALL}")
                return False
            
            print(f"{Fore.GREEN}Prostate agent initialization completed{Style.RESET_ALL}")
            return True
        ...
        try:
            ...
            if response and response.get('success'):
                ...
                if response.get('sources'):
                    print(f"\n{Fore.CYAN}Referenced {len(response['sources'])} sections from EAU Prostate Guidelines{Style.RESET_ALL}")
    ...
    def chat_mode(self):
        ...
        print(f"{Fore.YELLOW}Example: What are PSA levels for prostate cancer screening?{Style.RESET_ALL}\n")
    ...
    def init_system(self):
        """시스템 초기화"""
        print(f"{Fore.BLUE}Starting prostate system initialization...{Style.RESET_ALL}")
        ...
        try:
            ...
            if self.agent.initialize():
                print(f"{Fore.GREEN}Prostate system initialization completed{Style.RESET_ALL}")
                return True
            else:
                print(f"{Fore.RED}Prostate system initialization failed{Style.RESET_ALL}")
                return False
        except Exception as e:
    ...
    def show_status(self):
        """시스템 상태 확인"""
        print(f"{Fore.BLUE}Checking prostate system status...{Style.RESET_ALL}")
        ...
        try:
            ...
            print(f"\n{Fore.CYAN}Prostate System Status:{Style.RESET_ALL}")
            ...
            print(f"  • Model: {'Available' if status.get('model_available') else 'Not available'}")
    ...
    def run(self):
        ...
        parser = argparse.ArgumentParser(
            description='DR-Prostate-CLI - 전립선 질환 EAU 가이드라인 AI Agent',
        ...
        parser.add_argument('--model', '-m', default='gemma3:4b', help='Model to use (default: gemma3:4b)')
        ...
        if args.command == 'query':
            if not args.question:
                ...
                print(f"{Fore.YELLOW}Example: dr-prostate query \"What are PSA levels for prostate cancer screening?\"{Style.RESET_ALL}")
                return
